package com.tcore.scoreboard;

import com.tcore.utils.ScoreboardUtil;
...
public class FastBoard {
    ...
    private static final Object ENUM_SB_HEALTH_DISPLAY_HEARTS;
    ...
    static {
        try {
            ...
            if (VersionType.V1_8.isHigherOrEqual()) {
                ...
                ENUM_SB_HEALTH_DISPLAY_HEARTS = FastReflection.enumValueOf(ENUM_SB_HEALTH_DISPLAY, "HEARTS");
                ...
                ENUM_SB_HEALTH_DISPLAY_HEARTS = null;
    ...
    private final String belowId;

    private String title = ChatColor.RESET.toString();
    private List<String> lines = new ArrayList<>();

    private boolean deleted = false;
    private boolean belowObjective = false;
    ...
    public void updateUnderNameText(String text)  {

        if (!belowObjective) {
            try {
                sendHeartObjectivePacket(ObjectiveMode.CREATE, text);
                sendDisplayObjectiveHeartPacket();
                belowObjective = true;
            } catch (ReflectiveOperationException e){
                e.printStackTrace();
            }
        ...
        try {
           // sendHeartObjectivePacket(ObjectiveMode.UPDATE, text);

            sendScorePacket(0, ScoreboardAction.CHANGE, belowId);

            sendTeamPacket(0, TeamMode.UPDATE, player.getName());
    ...
    private void sendHeartObjectivePacket(ObjectiveMode mode, String text) throws ReflectiveOperationException {
        Object packet = PACKET_SB_OBJ.newInstance();

        setField(packet, String.class, belowId);
        setField(packet, int.class, mode.ordinal());

        if (mode != ObjectiveMode.REMOVE) {
            setComponentField(packet, text, 1);

            if (VersionType.V1_8.isHigherOrEqual()) {
                setField(packet, ENUM_SB_HEALTH_DISPLAY, ENUM_SB_HEALTH_DISPLAY_HEARTS);
            }
        } else if (VERSION_TYPE == VersionType.V1_7) {
            setField(packet, String.class, "", 1);
        }
    ...
    private void sendDisplayObjectiveHeartPacket() throws ReflectiveOperationException {
        Object packet = PACKET_SB_DISPLAY_OBJ.newInstance();


        setField(packet, int.class, 2);
        setField(packet, String.class, belowId);
    ...
    private void sendScoreHeartPacket(int score, ScoreboardAction action) throws ReflectiveOperationException {
        ...
        if (action == ScoreboardAction.CHANGE) {
            setField(packet, String.class, belowId, 1);
    ...
    private void sendScorePacket(int score, ScoreboardAction action) throws ReflectiveOperationException {
        sendScorePacket(score, action, id);
    }

    private void sendScorePacket(int score, ScoreboardAction action, String id) throws ReflectiveOperationException {
        Object packet = PACKET_SB_SCORE.newInstance();
    ...
    private void sendTeamPacket(int score, TeamMode mode) throws ReflectiveOperationException{
        sendTeamPacket(score, mode, id + ':' + score);
    }

    private void sendTeamPacket(int score, TeamMode mode, String teamName) throws ReflectiveOperationException {
        ...
        setField(packet, String.class, teamName); // TeamHandler name
        setField(packet, int.class, mode.ordinal(), VERSION_TYPE == VersionType.V1_8 ? 1 : 0); // Update mode

        if (mode == TeamMode.CREATE || mode == TeamMode.UPDATE) {
            String text = getLineByScore(score);
            ScoreboardUtil.Component cont = new ScoreboardUtil.Component(null, null);
            boolean add = false;
            if (text.length() > 16) {
                text = text.replace("§", "&");
                cont.setA(text.substring(0, 16));
                if (cont.getA().endsWith("§") || cont.getA().endsWith("&")){
                    cont.setA(cont.getA().substring(0, 15));
                    add = true;
                }
              //  cont.setB(ChatColor.getLastColors(text.substring(0, 16)) + text.substring(16));
            } else {
                cont.setA(text);
                cont.setB("");
            }

            String color = StringUtils.getLastColor(cont.getA(), '&', '§');
            if (text.length() > 16){
               if (add) cont.setB( "§" + ChatColor.translateAlternateColorCodes('&', text.substring(16)));
                else cont.setB(color.replace("&", "§") + ChatColor.translateAlternateColorCodes('&', text.substring(16)));
            }

            String prefix = cont.getA().replace("&", "§");
            String suffix =  cont.getB().replace("&", "§");

            if (VERSION_TYPE != VersionType.V1_13) {
                if (prefix.length() > 16 || (suffix != null && suffix.length() > 16)) {
                    throw new IllegalArgumentException("Line with score " + score + " is too long: " + text + '(' + prefix + '/' + suffix + ')');
                }
            }
